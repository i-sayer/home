        ...
        do {
            ...
            if (eventCode == 0xFF) {
                ...
                if (subCode == 0x2f)
                    break;
                if (subCode == 0x51){
                    var tempo_us = dataview.getUint32(this.fp)>>8;
                    mid$.tempo.push({bpm:tempo_us,time:this.currTime});
                }
                if (subCode == 0x58){
                    var tsnumerator = dataview.getInt8(this.fp);
                    var tsdenominator = Math.pow(2,dataview.getInt8(this.fp+1));
                    mid$.timesig = tsnumerator+"/"+tsdenominator ;
                }
                if (subCode == 0x03) {
                    var txt = "";
                    for (var i = 0; i < len; i++)
